    ...
    export const makePlayerRouter = (service: IPlayerService): Router => {
        ...
        playerRouter.get('/', async (req: Request, res: Response) => {
            ...
            const players = await service.getLatestPlayer({limit: Number(limit), before, after});

            return res.status(HttpStatus.OK).json(players);
        });

        playerRouter.post('/', BodyValidation(CreatePlayerDto), async (req: Request, res: Response) => {
